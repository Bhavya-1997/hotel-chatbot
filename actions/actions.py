    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if not name:
            ...
            if (len(user_input) > 1 and 
                ...
                dispatcher.utter_message(text=f"Hi {user_input}! When would you like to check in? (e.g., '20/9/2026', 'September 20')")
        ...
        if not checkin:
            ...
            if not parsed_date:
                dispatcher.utter_message(text="I couldn't understand that date. Please try '20/9/2026', 'September 20', or 'sep 20'.")
                return []
        ...
        if not checkout:
            ...
            parsed_date = self._parse_date(user_input)
            if not parsed_date:
                dispatcher.utter_message(text="I couldn't understand that date. Please try '23/9/2026', 'September 23', or 'sep 23'.")
                return []
...
Please choose: Standard, Deluxe, Family, or Suite."""
    ...
    def _parse_date(self, text: str):
        ...
        try:
            ...
            if match:
                result = datetime.strptime(f"{match[1]}/{match[2]}/{date.today().year}", "%d/%m/%Y").date()
            ...
            for month_name, month_num in months.items():
                ...
                if match:
                    result = date(date.today().year, month_num, int(match[1]))
